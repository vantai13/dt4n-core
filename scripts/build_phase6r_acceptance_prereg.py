...
def main() -> int:
    content = {
        ...
        "operational_definitions": {
            ...
            "S4_clock_implementation": {
                "start": "events[kind=inject].t_source - apply_ms/1000, the SAME source on Phase 5 and on the R-set (ml.acceptance_metrics.inject_clock)",
                "end": "snapshot.t_cycle_end of the first Transition into the alarm zone inside [inject_tick+1, revert_tick]: the detector cannot change state before the snapshot exists; Transition.t_source is the cycle START",
                "why_not_sidecar_interventions": "on R-D and R-C log_inject is False, so sidecar.interventions holds ONLY the revert at tick 40. Clocking from it gives negative times and a false PASS. amendment 5 analysis_rule governs SUPPRESSION, not this clock",
                "both_ends_self_penalising": "earliest physically grounded start, latest physically grounded end",
                "withdrawn": "'Phase 5 detections sit at 1.1-2.1 s' used nominal t_rel 20.0 against measured times; rehearsal on this clock gives 994-1999 ms (envelope_only)",
            },
