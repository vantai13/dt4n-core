...
def main() -> int:
    content = {
        ...
        "operational_definitions": {
            ...
            "S2_estimator": "rule of three: 0 events over T hours gives a 95% upper bound of 3/T. With T = 3 hours that is 1.0 events/hour.",
            "S2_estimator_if_nonzero": "exact Poisson upper bound, alpha = 0.05",
        ...
        "analysis_commitments": {
            "dose_axis_1": {
                ...
                "why_cluster": "ticks inside one run share a seed, a link, a dose and a network state, so they are not independent. Resampling ticks would assume ~400 independent observations instead of ~10 and narrow the CI by roughly sqrt(ticks per run).",
        ...
        "results_skeleton_committed_first": {
            ...
            "test": "set(keys(skeleton)) == set(keys(filled))",
        },
        "read_once_guard": {
            "rule": "read_snapshots_once(path) raises on a second read of the same resolved path",
            "receipt_field": "n_files_read",
        },
        "invocation_log": {
            "file": "results/report/phase6r_acceptance_runs.log",
            "rule": "append-only and committed; the receipt reads it back to fill n_acceptance_invocations",
